...
class Header_Walker extends Walker_Nav_Menu
{
	...
	private function is_home_link($item)
	{
		return '/home' === untrailingslashit((string) wp_parse_url($item->url ?? '', PHP_URL_PATH));
	}
	...
	private function is_current_item($item)
	{
		...
		if (is_front_page() && $this->is_home_link($item)) {
			return true;
		}
	...
	function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
	{
		...
		if ($depth == 0) {
			if ($has_kids) {
				...
				$output .= $indent . '<li class="' . esc_attr($item_class) . '">';
				$output .= '<a class="site-header__menu-link" href="' . esc_url($item_url) . '"' . $curr . '>';
				$output .= esc_html($t);
				$output .= '</a>';
			}
		} elseif ($depth == 1) {
			...
			$output .= '<a class="site-header__dropdown-link" href="' . esc_url($item_url) . '"' . $curr . '>';
			$output .= esc_html($t);
			$output .= '</a>';
		}
	}
