...
/**
 * Customize Contact Form 7 submit button to use <button> tag with SVG icon.
 */
function appian_customize_cf7_submit_button($content) {
	// Find <input type="submit" ...> tags
	$content = preg_replace_callback(
		'/<input\s+([^>]*?)type=["\']submit["\']([^>]*?)>/i',
		function ($matches) {
			$attrs = $matches[1] . ' ' . $matches[2];

			// Extract the value attribute (which is the button text)
			$value = 'Submit';
			if (preg_match('/value=["\'](.*?)["\']/i', $attrs, $val_match)) {
				$value = $val_match[1];
			}
			...
			// Extract other attributes like id, disabled, style, etc., but exclude type, class, value
			$other_attrs = preg_replace('/(class|value|type)=["\'].*?["\']/i', '', $attrs);
			// Clean up whitespace
			$other_attrs = preg_replace('/\s+/', ' ', trim($other_attrs));
			if (!empty($other_attrs)) {
				$other_attrs = ' ' . $other_attrs;
			}

			// Get the arrow-right SVG markup
			$svg = '';
			if (function_exists('appian_get_svg_icon')) {
				$svg = appian_get_svg_icon('arrow-right');
			}

			// Return button tag instead
			return sprintf(
				'<button type="submit" class="%s"%s><span>%s</span>%s</button>',
				esc_attr($classes),
				$other_attrs,
				esc_html($value),
				$svg
...
add_filter('wpcf7_form_elements', 'appian_customize_cf7_submit_button');