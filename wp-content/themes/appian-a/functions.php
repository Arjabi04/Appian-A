...
add_action('acf/input/admin_footer', function () {
    global $post;
    // Only run this on screens where the Hero Projects block could appear
    ?>
    <script type="text/javascript">
    (function($) {
        function checkImagePositionConflict($field) {
            var val = parseInt($field.find('input[type="number"]').val(), 10);
            var $notice = $field.find('.image-position-warning');

            if ($notice.length === 0) {
                $field.append('<p class="image-position-warning acf-notice -warning" style="display:none; margin-top:8px;">⚠ This position will overlap the card before it (the feature image spans 2 columns and cannot start at the last column of a row). Choose a different position.</p>');
                $notice = $field.find('.image-position-warning');
            }
        ...
        acf.addAction('ready_field/name=hero_projects_image_position', function($field) {
            checkImagePositionConflict($field);
            $field.find('input[type="number"]').on('input', function() {
                checkImagePositionConflict($field);
            });
        });

        acf.addAction('append_field/name=hero_projects_image_position', function($field) {
            checkImagePositionConflict($field);
            $field.find('input[type="number"]').on('input', function() {
                checkImagePositionConflict($field);
            });
